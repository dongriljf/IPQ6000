...
if [ -f "$PW_FILE" ]; then
	...
	sed -i '/config PACKAGE_$(PKG_NAME)_INCLUDE_Shadowsocks /,/default n/d' $PW_FILE
	sed -i '/Shadowsocks_NONE/d; /Shadowsocks_Libev/d' $PW_FILE
...
PW2_FILE=$(find ./ -maxdepth 3 -type f -wholename "*/luci-app-passwall2/Makefile")
if [ -f "$PW2_FILE" ]; then
	sed -i '/config PACKAGE_$(PKG_NAME)_INCLUDE_Shadowsocks_Libev/,/x86_64/d' $PW2_FILE
	sed -i '/config PACKAGE_$(PKG_NAME)_INCLUDE_Shadowsocks /,/default n/d' $PW2_FILE
	sed -i '/Shadowsocks_NONE/d; /Shadowsocks_Libev/d' $PW2_FILE
...
if [ -f "$SP_FILE" ]; then
	...
	sed -i '/config PACKAGE_$(PKG_NAME)_INCLUDE_Shadowsocks /,/x86_64/d' $SP_FILE
	sed -i '/Shadowsocks_NONE/d; /Shadowsocks_Libev/d' $SP_FILE
